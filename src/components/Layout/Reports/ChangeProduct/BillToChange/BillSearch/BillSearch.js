...
export const BillSearch = (props) => {
  ...
  const onSubmit = () => {
    search({
      ...
      onSuccess: (d) => {
        setCart({...cart,bill_reference:d,customer:d.customer.id})
        setBillNumber("")
      }
    })
  }
  return (
    <Form onFinish={onSubmit}>
      <Item>
        <Input placeholder="Entrez le numero de la Facture" value={billNumber} onChange={e => setBillNumber(e.target.value)}
