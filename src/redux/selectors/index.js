...
export const selectReadEntities = (state, entityName, params) => {
  const readId = state.entities[entityName].readIds[JSON.stringify(params)];
  if (!readId) {
    return [];
  }
...
export const selectPagination = (state, entityName, id, key) => {
  const pager = state.entities[entityName][key];
  return hasKey(pager, id) ? pager[id].pagination : undefined;
};
...
export const selectReadEntitiesPagination = (state, entityName, params) => (
  selectPagination(state, entityName, JSON.stringify(params), 'readIds')
);
...
export const selectCreatedEntity = (state, entityName, uuid) => {
  ...
  const { items } = createIds[createIdKey];
  if (items) {
    return selectEntity(state, entityName, items);
  }
  return undefined;
};
